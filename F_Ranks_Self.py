...
def save_and_display_results(name, visited, correct_answers, total_time):
    """Save player's results to results_self.json and display ranked leaderboard."""
    result_self = {
    ...
    try:
        with open("results_self.json", "r", encoding="utf-8") as file:
            data = json.load(file)
    ...
    with open("results_self.json", "w", encoding="utf-8") as file:
